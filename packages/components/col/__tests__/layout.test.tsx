...
import Row from '@forward-ui/components/row/src/row.vue'
import Col from '../src/col.vue'

describe('Col', () => {
  it('create', () => {
    const wrapper = mount(() => <Col />)
    expect(wrapper.classes()).toContain('f-col')
  })

  it('span', () => {
    const wrapper = mount(() => <Col span={12} />)
    expect(wrapper.classes()).toContain('f-col-12')
  })

  it('pull', () => {
    const wrapper = mount(() => <Col span={12} pull={3} />)
    expect(wrapper.classes()).toContain('f-col-pull-3')
  })

  it('push', () => {
    const wrapper = mount(() => <Col span={12} push={3} />)
    expect(wrapper.classes()).toContain('f-col-push-3')
  })
  ...
  it('responsive', () => {
    ...
    expect(colElmClass.includes('f-col-sm-4')).toBe(true)
    expect(colElmClass.includes('f-col-sm-4')).toBe(true)
    expect(colElmClass.includes('f-col-offset-sm-2')).toBe(true)
    expect(colElmClass.includes('f-col-lg-6')).toBe(true)
    expect(colElmClass.includes('f-col-offset-lg-3')).toBe(true)
    expect(colElmClass.includes('f-col-md-8')).toBe(true)
  })
})
...
describe('Row', () => {
  test('create', () => {
    ...
    expect(wrapper.classes()).toContain('f-row')
  })
