...
// import { ElFormItem } from '@forward-ui/components/form'
import Switch from '../src/switch.vue'
import type { VueWrapper } from '@vue/test-utils'
import type { SwitchInstance } from '../src/message'
...
describe('Switch.vue', () => {
    ...
    test('create', () => {
        ...
        const vm = wrapper.vm
        expect(vm.$el.style.getPropertyValue('--f-switch-on-color')).toEqual(
            '#0f0'
        )
        expect(vm.$el.style.getPropertyValue('--f-switch-off-color')).toEqual(
            '#f00'
        )
        expect(vm.$el.classList.contains('is-checked')).false
        const coreEl = vm.$el.querySelector('.f-switch__core')
        expect(coreEl.style.width).toEqual('100px')
        const leftLabelWrapper = wrapper.find('.f-switch__label--left span')
        expect(leftLabelWrapper.text()).toEqual('off')
    })

    test('size', () => {
        const wrapper = mount(() => <Switch size="large" />)
        expect(wrapper.find('.f-switch--large').exists()).toBe(true)
    })

    test('tabindex', () => {
        const wrapper = mount(() => <Switch tabindex="0" />)
        expect(wrapper.find('.f-switch__input').attributes().tabindex).toBe('0')
    })
    ...
    test('inline prompt', () => {
        ...
        const coreEl = vm.$el.querySelector('.f-switch__core')
        expect(coreEl.style.width).toEqual('100px')
        const label = wrapper.find('.f-switch__inner span')
    ...
    test('value correctly update', async () => {
        ...
        const coreWrapper = wrapper.find('.f-switch__core')
